...
* **智慧過濾機制 (SmartIgnore)**：完美支援標準 `.gitignore` 規則！系統預設攔截 `__pycache__`、`venv` 及編譯好的 CLI 執行檔，更會自動解析專案下的 `.gitignore` 檔案。無論是在本地比對狀態或批次上傳，都能精準略過垃圾檔案，確保倉庫乾淨無虞。
...
├── .gitignore           # 🌟 智慧忽略配置檔 (定義 CLI 與 Web 排除追蹤的垃圾檔案與環境目錄)
...
純粹比對本地與伺服器的檔案差異狀態，**絕對不上傳任何資料**。*(已內建 `.gitignore` 智慧過濾，自動忽略系統與快取檔案)*
...
智慧比對差異並精準批次上傳。自動在網頁端留下多行異動履歷。*(支援 `.gitignore`，批次提交 `.` 時不會誤傳垃圾檔案)*
